...
#### 盘搜
夸克/115/百度/UC网盘资源搜索。群内发送 `@机器人 搜索 <关键词>` 即可。管理员可单独开关。需自建盘搜 API 服务。

#### TG 转发
Telegram 频道消息自动转发到指定微信群。支持文字、图片、视频、文件。管理员可单独开关。自动添加新授权群到转发配置。需创建 Telegram Bot 并配置。

#### CFTC 上传
群内发送图片/文件后发送 `@机器人 上传` 自动上传到图床。管理员可单独开关。需自建 CFTC 服务。

#### LSPosed 模块更新追踪
监控 Xposed 模块市场及自定义 GitHub 仓库，有新版本时自动推送到微信群。管理员可单独开关。无需额外服务。

#### 公众号推文
通过 [WeRSS](https://github.com/rachelos/we-mp-rss) 订阅微信公众号，有新文章时自动推送到微信群。管理员可单独开关。需自行部署 WeRSS 服务。
...
- 需要自行部署 [WeRSS](https://github.com/rachelos/we-mp-rss) 服务
